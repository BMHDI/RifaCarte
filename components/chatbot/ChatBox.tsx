...
export function ChatBox() {
  ...
  const welcomeMessage: Message = {
    role: 'assistant',
    content:
      'Bonjour ! Je suis votre assistant pour chercher des services francophones. Comment puis-je vous aider ?',
    ...
    <div className="flex flex-col h-[80vh] rounded-xl bg-white  overflow-hidden">
      ...
      {messages.length > 1 && (
        <div
          ...
          className="flex justify-end items-center   flex gap-2 items-center text-primary hover:text-black transition-colors "
        >
          Nouvelle discussion{' '}
            ...
            >
              <div className="text-sm leading-relaxed whitespace-pre-wrap">
                <ReactMarkdown
                  ...
                  components={{
                    ul: ({ children }) => (
                      <ul className="list-disc list-inside ml-4 space-y-0.5">{children}</ul>
                    ),
                    ol: ({ children }) => (
                      <ol className="list-decimal list-inside ml-4 space-y-0.5">{children}</ol>
                    ),
