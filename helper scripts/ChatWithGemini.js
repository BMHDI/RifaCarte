...
function searchOrgs(query, topN = 5) {
  ...
  const scoredOrgs = orgs.map((org) => {
    let score = 0;
    const fields = [org.name, ...(org.services || []), ...(org.tags || []), ...(org.category || [])]
      .join(' ')
      .toLowerCase();
...
async function chatAvecOrgs(userQuery) {
  ...
  const res = await fetch('https://gemini.googleapis.com/v1/chat/completions', {
    method: 'POST',
    headers: {
...
(async () => {
  await chatAvecOrgs('Je cherche du soutien pour les aînés francophones à Edmonton');
})();
